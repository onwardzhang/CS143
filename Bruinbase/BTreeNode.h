...
//preserve sizeof(int) for keyCount, sizeof(PageId) for pointer to sibling page
const int PRESERVED_SPACE = sizeof(int) + sizeof(PageId);//8
const int LEAF_ENTRY_SIZE = sizeof(int) + sizeof(RecordId);//4+4+4 = 12
const int NONLEAF_ENTRY_SIZE = sizeof(int) + sizeof(PageId);//8
const int LEAF_MAX_KEY_COUNT = (PageFile::PAGE_SIZE - PRESERVED_SPACE) / LEAF_ENTRY_SIZE;//84
const int NONLEAF_MAX_KEY_COUNT = (PageFile::PAGE_SIZE - PRESERVED_SPACE) / NONLEAF_ENTRY_SIZE;//127
/**
...
class BTLeafNode {
  public:
    ...
    */
    ...
    void changeBuffer(int offset, const void * newContent, size_t size) {
        memcpy(buffer + offset, newContent, size);
    }
